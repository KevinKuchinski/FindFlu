...
def main():
    ...
    delete_blast_results_file(args.db_file, args.input_file, args.output_dir, args.output_name)
...
def blast_sequences(db_file_path, query_file_path, output_dir, output_name, blast_threads):
    ...
    blast_results_path = os.path.join(output_dir,
                                      f'{query_file_path}_{db_file_path}_{output_name}_blast_results.tsv')
...
def delete_blast_results_file(db_file_path, query_file_path, output_dir, output_name):
    blast_results_path = os.path.join(output_dir,
                                      f'{query_file_path}_{db_file_path}_{output_name}_blast_results.tsv')
...
def filter_fragments_by_best_matches(blast_results):
    ...
    subject_annots['subtype'] = subject_annots.apply(lambda row: row['segment'] if row['subtype'] == 'none' else row['subtype'], axis=1)
...
def annotate_blast_results(blast_results):
    ...
    blast_results = pd.merge(blast_results, subject_annots, on='sseqid')
    for segment in ['PB2', 'PB1', 'PA', 'NP', 'M', 'NS']:
        blast_results['subtype'] = blast_results['subtype'].replace(segment, 'none')
    return blast_results
...
def calc_combined_id_and_cov(blast_results):
    ...
    group_cols = ['experiment', 'library', 'fragment', 'sseqid']
    blast_results = pd.merge(blast_results, combined_id_and_cov[cols], on=group_cols)
    return blast_results
...
def calc_frag_length(blast_results):
    ...
    #
    blast_results = pd.merge(blast_results, alignment_coordinates, on=group_cols)
    return blast_results
...
def write_best_ref_seqs_report(blast_results, output_dir, output_name):
    ...
    cols = ['experiment', 'library', 'fragment', 'UMI_pair', 'ref_seq_ID', 'ref_seq', 'segment', 'subtype',
            'combined_query_id', 'combined_query_cov', 'frag_subject_cov']
